...
export async function POST(request: Request) {
  try {
    ...
    try {
      ...
      return NextResponse.json({ message: "Request received and stored." }, { status: 201 });
    } catch (dbErr) {
      ...
      console.error("Account deletion request received but DB failed:", dbErr, payload);
      return NextResponse.json({ message: "Request received (not stored: DB unavailable)." }, { status: 202 });
    }
  } catch (err) {
    // eslint-disable-next-line no-console
